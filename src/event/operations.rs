...
pub fn print(scope: &mut Stack, params: &Vec<Variable>) {
...
pub fn set_activeness(scope: &mut Stack, action_activeness: &mut HashMap<String,bool>, params: &Vec<Variable>, val: bool) {
...
pub fn toggle_activeness(scope: &mut Stack, action_activeness: &mut HashMap<String,bool>, params: &Vec<Variable>) {
    let op_name = "toggle";
    expect_param_count(op_name, params, 1);
...
pub fn add(scope: &mut Stack, params: &mut Vec<Variable>) {
...
pub fn sub(scope: &mut Stack, params: &mut Vec<Variable>) {
...
pub fn set(scope: &mut Stack, params: &mut Vec<Variable>) {
    expect_param_count("set", params, 2);
    let v2 = &params[1];
    let new_val = v2.get_value(scope);
    let v1 = &mut params[0];
    v1.set_value(scope, new_val);
}

pub fn top_into(scope: &mut Stack, params: &mut Vec<Variable>) {
    expect_param_count("top into", params, 2);
    let par1 = &params[0].get_value(scope);
    let v = par1.into_vec();
    if v.is_empty() { panic!("error: empty vector"); }
    let top = v[0].get_value(scope);
    params[1].set_value(scope, top);
}

pub fn rotate_vec(scope: &mut Stack, params: &mut Vec<Variable>) {
...
pub fn move_by(scope: &mut Stack, params: &mut Vec<Variable>) {
